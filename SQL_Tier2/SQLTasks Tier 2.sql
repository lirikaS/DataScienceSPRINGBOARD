...
SELECT `facid`, `name`, `membercost`, `monthlymaintenance` FROM `Facilities` WHERE `membercost` > 0 and `membercost` < 0.2*(`monthlymaintenance`*0.2);
...
SELECT f.name, b.starttime AS month, COUNT(b.bookid) AS usage_count
FROM Facilities AS f
JOIN Bookings AS b ON f.facid = b.facid
WHERE b.memid != 0
GROUP BY f.name, month;

SELECT f.name, strftime('%m', b.starttime) AS month, COUNT(b.bookid) AS usage_count
FROM Facilities AS f
JOIN Bookings AS b ON f.facid = b.facid
WHERE b.memid != 0
GROUP BY f.name, month;
